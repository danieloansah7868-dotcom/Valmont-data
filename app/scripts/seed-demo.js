...
const { buildDemo, summarize, DEMO_CUSTOMERS } = require("../lib/demo-data");
...
function toSql(data, { now }) {
  ...
  P(`-- ============================================================================`);
  P(`-- VALMONT DATA — demo seed (customers, saved numbers, orders, float, webhooks)`);
  ...
  const s = summarize(data);
  P(`  raise notice 'Valmont Data demo seed complete: % customers, % saved numbers, % orders, % float entries, % webhook logs (final float: MTN %, Telecel %, AirtelTigo %).',`);
  P(`    ${s.customers}, ${s.saved_numbers}, ${s.orders}, ${s.float_ledger_entries}, ${s.webhook_logs},`);
...
if (args.sql || args["sql-out"]) {
  ...
  console.log(`  (${data.orders.length} orders, ${data.float_ledger.length} float entries, ${data.webhook_log.length} webhook logs — anchored to ${asOf.toISOString()})`);
} else {
